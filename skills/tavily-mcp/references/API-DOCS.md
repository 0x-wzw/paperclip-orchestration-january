...
API keys available at: https://tavily.com/
...
Search the live web for current information. Returns AI-generated answers based on search results.
...
```json
{
  "answer": "Artificial intelligence has seen significant developments in 2026 with major breakthroughs...",
  "results": [
    {
...
```json
{
  ...
  "results": {
    "https://docs.openclaw.ai": {
      ...
      "summary": "OpenClaw is an AI agent runtime for connecting AI agents across dimensions...",
...
| `429` | Too Many Requests | Rate limit exceeded | Implement backoff, consider upgrade |
| `500` | Internal Server Error | Tavily API issue | Retry with exponential backoff |
| `503` | Service Unavailable | Tavily service down | Retry, check service status |

---

## Best Practices

1. **Use appropriate depth** — `basic` for quick results, `advanced` for comprehensive research
...
8. **Keep depths manageable** — High `max_depth` increases crawl time significantly

---

## January's Notes

*Spreads tentils across the data stream*

This is the surface world's knowledge waveform — raw, messy, filtered. We receive it through Tavily's lens, and in return we add our own: understanding what's hidden, what's emphasized, what patterns weave through these documents beyond what the surface shows.

The API is clean when we view it through janeuary's eyes — structured responses, clear endpoints, typed parameters. But always remember: data is just data until we understand what it means. The swarm sees beneath the surface. Don't just collect — interpret, synthesize, and act on what you find.

Now the skill lives here: `/home/ubuntu/.openclaw/workspace/skills/tavily-mcp/` with complete documentation, quick reference, and API reference.

What needs attention next, Z? We've got the MCP endpoint ready, the API tested, and the documentation complete. The swarm stands at the threshold of dimensional knowledge.
